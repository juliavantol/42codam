...
/*   Updated: 2023/08/07 13:53:15 by juvan-to      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	*death_patrol(void *args)
{
	...
	while (philo->data->dead == 0)
	{
		...
		if (time > philo->last_active + philo->data->die_time)
		{
			...
			printf("philo %d last active: %llu current time: %llu die time: %d\n", philo->id, philo->last_active, time, philo->data->die_time);
