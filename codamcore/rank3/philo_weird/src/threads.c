...
/*   Updated: 2023/08/20 19:46:00 by Julia         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	init_threads(t_data	*data, int index)
{
	pthread_t	p;

	if (pthread_create(&p, NULL, &supervisor, (void *)data) != 0)
		return (ft_error(data, "Error: create thread", 2));
	...
	while (index < data->philo_count)
	{
		if (pthread_join(data->threads[index++], NULL) != 0)
			return (ft_error(data, "Error: join thread", 2));
	}
	if (pthread_join(p, NULL) != 0)
		return (ft_error(data, "Error: join thread", 2));
	return (0);
